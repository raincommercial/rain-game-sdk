...
describe("Rain1155 Test", function () {
  ...
  it("Should mint multiple assets",async () => {
    ...
    expect(await USDT.balanceOf(buyer2.address)).to.deep.equals(ethers.BigNumber.from("0" + eighteenZeros))
    expect(await BNB.balanceOf(buyer2.address)).to.deep.equals(ethers.BigNumber.from("0" + eighteenZeros))
    expect(await CARS.balanceOf(buyer2.address, 5)).to.deep.equals(ethers.BigNumber.from("0"))
    expect(await PLANES.balanceOf(buyer2.address, 15)).to.deep.equals(ethers.BigNumber.from("0"))
