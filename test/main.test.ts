...
describe('Rain1155 Test', function () {
  ...
  it("Should create asset '1' from creator. [AND - OR gating rules ((A && B) || (C && D)) ]", async function () {
    ...
    const [vmStateConfig, currencies] = Rain1155SDK.generateScript(
      [conditions1, conditions2, conditions3],
      prices
    );
    const [canMintConfig, priceConfig] = Rain1155SDK.generateConfig(
      vmStateConfig,
      currencies
    );
  ...
  it("Should buy asset '1'", async function () {
    ...
    expect(await rain1155SDK.balanceOf(buyer1.address, assetId)).to.deep.equals(
    ...
    expect(await USDT.balanceOf(buyer1.address)).to.deep.equals(
      ethers.BigNumber.from('14' + eighteenZeros)
    );
  ...
  it("Should not be able to buy asset '1' after failing to satisfy the gating rules.", async function () {
    ...
    let USDTPrice = (
      await rain1155SDK.getAssetPrice(assetId, USDT.address, 1)
    )[1];
    let BNBPrice = (
      await rain1155SDK.getAssetPrice(assetId, BNB.address, 1)
    )[1];
    ...
    await expect(
      rain1155SDK.connect(buyer2).mintAssets(assetId, 1)
    ).to.revertedWith('Cant Mint');

    expect(await rain1155SDK.balanceOf(buyer2.address, assetId)).to.deep.equals(
      ethers.BigNumber.from('0')
    );
  });
  ...
  it("Should buy asset '2'", async function () {
    ...
    expect(await rain1155SDK.balanceOf(buyer3.address, assetId)).to.deep.equals(
      ethers.BigNumber.from('1')
    );

    expect(await USDT.balanceOf(creator.address)).to.deep.equals(
      ethers.BigNumber.from('2' + eighteenZeros)
    );
    expect(await BNB.balanceOf(creator.address)).to.deep.equals(
      ethers.BigNumber.from('50' + eighteenZeros)
    );
    expect(await CARS.balanceOf(creator.address, 5)).to.deep.equals(
    ...
    expect(await USDT.balanceOf(buyer3.address)).to.deep.equals(
      ethers.BigNumber.from('14' + eighteenZeros)
    );
    expect(await BNB.balanceOf(buyer3.address)).to.deep.equals(
      ethers.BigNumber.from('0' + eighteenZeros)
    );
    expect(await CARS.balanceOf(buyer3.address, 5)).to.deep.equals(
      ethers.BigNumber.from('0')
    );
    expect(await PLANES.balanceOf(buyer3.address, 15)).to.deep.equals(
      ethers.BigNumber.from('0')
    );
  });
  ...
  it("Should not be able to buy asset '2' after failing to satisfy the gating rules.", async function () {
    ...
    expect(await rain1155SDK.balanceOf(buyer4.address, assetId)).to.deep.equals(
      ethers.BigNumber.from('0')
    );
  });
  ...
  it("Should buy asset '3'", async function () {
    ...
    expect(await rain1155SDK.balanceOf(buyer5.address, 3)).to.deep.equals(
      ethers.BigNumber.from('1')
    );

    expect(await USDT.balanceOf(creator.address)).to.deep.equals(
      ethers.BigNumber.from('3' + eighteenZeros)
    );
    expect(await BNB.balanceOf(creator.address)).to.deep.equals(
      ethers.BigNumber.from('55' + eighteenZeros)
    );
    expect(await CARS.balanceOf(creator.address, 5)).to.deep.equals(
    ...
    expect(await USDT.balanceOf(buyer5.address)).to.deep.equals(
      ethers.BigNumber.from('4' + eighteenZeros)
    );
    expect(await BNB.balanceOf(buyer5.address)).to.deep.equals(
      ethers.BigNumber.from('5' + eighteenZeros)
    );
    expect(await CARS.balanceOf(buyer5.address, 5)).to.deep.equals(
      ethers.BigNumber.from('0')
    );
    expect(await PLANES.balanceOf(buyer5.address, 15)).to.deep.equals(
      ethers.BigNumber.from('0')
    );
  });
  ...
  it("Should not be able to buy asset '3' after failing to satisfy the gating rules.", async function () {
    ...
    await expect(rain1155SDK.connect(buyer6).mintAssets(3, 1)).to.revertedWith(
      'Cant Mint'
    );

    expect(await rain1155SDK.balanceOf(buyer6.address, 3)).to.deep.equals(
      ethers.BigNumber.from('0')
    );
  });
  ...
  it('Should mint multiple assets', async () => {
    ...
    expect(await rain1155.balanceOf(buyer2.address, 1)).to.deep.equals(
      ethers.BigNumber.from('4')
    );

    expect(await USDT.balanceOf(buyer2.address)).to.deep.equals(
      ethers.BigNumber.from('9' + eighteenZeros)
    );
    expect(await BNB.balanceOf(buyer2.address)).to.deep.equals(
      ethers.BigNumber.from('25' + eighteenZeros)
    );
    expect(await CARS.balanceOf(buyer2.address, 5)).to.deep.equals(
      ethers.BigNumber.from('10')
    );
    expect(await PLANES.balanceOf(buyer2.address, 15)).to.deep.equals(
      ethers.BigNumber.from('5')
    );
  });
});
